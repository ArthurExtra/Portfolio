...
13. When a branch needs deeper work, use subagents for focused tasks such as
    ...
    review of anti-analysis logic. Spawn only after the workspace, database, and
    current notes are up to date. Each subagent gets one bounded question and must
    return evidence paths plus database-ready rows; reconcile those rows into the
    shared workspace before launching more work. Good subagent splits:
...
15. **Maintain an exhaustive coverage queue.** Do not stop at the first finding or the obvious paths. Queue every exported component, deep link, content provider, WebView, shared pref, SQLite database, endpoint, auth flow, feature screen, and settings toggle. Work the queue in small proof loops and persist each result before taking the next item.
...
All paths relative to repo root (`/home/ad/System`):
