  ...
  home = {
    ...
    activation.createJSWorkspace = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
      ...
      if [ -f "$HOME/.local/share/.bun-global-installed" ]; then
        echo "✔ Global npm packages already installed, skipping global installation step"
      else
        echo "📦 Managing global npm packages with bun..."
        if $DRY_RUN_CMD ${pkgs.bun}/bin/bun add --global --cwd "$HOME" --no-summary ${lib.escapeShellArgs globalNpmPackages}; then
          $DRY_RUN_CMD touch "$HOME/.local/share/.bun-global-installed"
        else
          echo "❌ Failed to manage global npm packages"
        fi
      fi

      # Remove retired standalone Gemini CLI;
      # Antigravity CLI is Nix-managed as agy.
